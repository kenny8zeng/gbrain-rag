...
- **Wikilink knowledge graph**: write `[[Concept]]` in your Markdown and the service auto-creates the concept's node page plus the graph edges on import — then retrieve along it: `retrieval` takes an optional `graph` block that expands from the vector hits into **documents sharing those concepts** (returned with `via_concepts` provenance), and tenants can walk the graph directly via `GET /v1/kb/{id}/graph/traverse`. Entity nodes stay invisible to the document APIs (type + prefix filtering), so "page = your document" keeps holding
- **Bulk import**: ship a whole Markdown tree as one `tar` archive (`POST /v1/kb/{id}/documents/bulk`, any format `tar -xf` auto-detects) — the server derives slugs with the same rule as per-document import, materializes `[[wikilink]]` entity nodes, runs a single `gbrain import` plus one idempotent `extract links` pass; `dry_run=true` returns the slug mapping before you commit. Measured: a 185-doc replay drops from ~15 min to ~70 s
- **Standalone document parsing**: reuse the service's parsing ability on its own via `POST /v1/kb/parse` (file / web URL / plain-text pass-through) with **zero writes** — no pages, graph edges or jobs, no embedding quota. It follows the **same deployment-level parser priority** as knowledge-base import (first-choice parser + external parse service + fallback) and callers cannot override it; the response reports which parser actually ran. File-type acceptance is **content-first** (extension only as fallback). When no external parse service is configured, out-of-range types return `UNSUPPORTED_FILE_TYPE` and images/URLs return `PARSER_UNAVAILABLE` (never retry), while saturation returns `PARSE_BUSY` (503, retryable); `GET /health` exposes a `parse` capability block so callers can pre-check type support
- **Bounded engine capacity**: every engine call is a separate process, so all spawns pass through a global concurrency gate (`GBRAIN_CLI_CONCURRENCY`) with a bounded queue — saturation returns a retryable `503 UPSTREAM_BUSY` instead of collapsing under bulk import
