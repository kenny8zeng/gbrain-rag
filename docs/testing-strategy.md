...
| D33 | **解析链缓存键不完整**——`resolveParserFor` 按 `${PARSER_MODE}\|${DOCLING_URL}` 缓存单例，**漏掉 `PARSER_PREFERENCE`**：同一进程内变更偏好会复用旧链，首选/回退方向错误，且 `/health` 能力自描述与实际解析行为不一致（首选项与回退项互换） | 缓存键补入 `PARSER_PREFERENCE`（覆盖全部影响链形态的配置项） | `tests/unit/resolver-cache.test.ts`（9 例：双向偏好切换互不污染、同配置仍命中单例、PARSER_MODE/DOCLING_URL 仍在键内、生效矩阵 5 行对照文档；回退即 2 项失败） | ✓ |
| D32 | **凭证无法零知识库绑定**——`issueKey` 以 `opts.writeKb ?? opts.readKbs[0]` 推导上游 client 的 `--source`；`read_kbs: []` 时该值为 `undefined`，引擎拒绝注册 → 签发 500 `INTERNAL`。这使「仅需有效凭证、不绑定知识库」的裸解析（FR-010/SC-001）无法落地 | `readKbs` 为空时不创建上游 client（`client_id`/`client_secret` 置空，服务层凭证仅用于不依赖知识库的端点；对知识库端点仍 403）。`revokeKey` 原有 `if (row.clientId)` 守卫已兼容零绑定吊销 | `tests/integration/parse-api.test.ts`（零绑定凭证解析成功 + 知识库零副作用；两部署形态各 15 例） | ✓ |
