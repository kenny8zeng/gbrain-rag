...
> **外部解析服务的接线要求**：该地址是服务的**外部依赖**——其可达性、吞吐与配额直接决定图片/网页类解析的成败。两条实践要求：
> 1. **优先同网内地址**（容器网络/内网域名）。跨公网接线会为每次回退探测叠加 RTT（实测跨公网约 1.5–2s/次），且图片/网页解析必然经此通道。
> 2. **界面之外的含义**：若该服务需鉴权，凭证经 `DOCLING_URL` 无法传递（本项目不注入额外头）——故只应接入**部署内受信**的服务端点。文档内容会传送至该地址（含 OCR 场景），须符合数据出境/保密要求。
...
- **触发方式**：`DREAM_CRON` cron 表达式——默认 `0 4 * * *`（每日凌晨 4 点）；支持任意 5 段 cron（如每周一 `0 4 * * 1`、每小时 `0 * * * *`）；空 = 仅手工触发（`POST /v1/admin/dream`）
...
管理面接口：`POST /v1/admin/dream` 手工触发一次（异步 202；运行中再触发 409 `DREAM_RUNNING`——绝不叠跑）；`GET /v1/admin/dream` 查看状态（启用/档位/运行中/上次结果/下次计划）。运行互斥：执行中定时到点跳过顺延；超 4 小时强杀解锁；服务重启自动清锁。
...
- **批量导入**（`POST /v1/kb/:id/documents/bulk`）：单次 `gbrain import` 进程（占 CLI 闸门 1 槽，进程内 `--workers 4` 并行 embed），是大数据量灌库的首选路径。运行镜像需含 `gzip/zstd/bzip2/xz`（Dockerfile 已装，对应 tar 自动探测格式）。文件数上限 `BULK_MAX_FILES`（默认 500）
- 备份建议：
