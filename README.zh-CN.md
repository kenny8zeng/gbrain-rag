...
- **双链知识图谱**：Markdown 里写 `[[概念]]`，导入时自动创建概念节点页并连边——检索时可沿图走：`retrieval` 支持可选 `graph` 参数，从向量命中出发扩展出**同概念的其他文档**（带 `via_concepts` 溯源）；租户也可直接走 `GET /v1/kb/{id}/graph/traverse`。实体节点对文档接口**不可见**（类型 + 前缀双重过滤），"页面即你的文档"语义不变
- **批量导入**：整棵 Markdown 目录树打成一个 tar 归档（`POST /v1/kb/{id}/documents/bulk`，支持 `tar -xf` 可自动探测的格式）——服务端用与逐篇导入相同的规则派生 slug、自动落地 `[[双链]]` 实体节点、单次 `gbrain import` 加一次幂等 `extract links` 建边；`dry_run=true` 可先取回 slug 映射自检。实测 185 篇重放从 ~15 分钟降到 ~70 秒
- **裸文档解析**：通过 `POST /v1/kb/parse` 单独复用本服务的解析能力（文件 / 网页地址 / 纯文本直通），**零写入**——不产生页面、图谱边或任务，不消耗嵌入配额。解析优先级与知识库导入**同一套部署级设定**（首选解析器 + 外部解析服务 + 回退），调用方不可覆盖；响应如实报告实际生效的解析器。类型受理**内容优先**（扩展名仅作回退）。未配置外部解析服务时，范围外类型返回 `UNSUPPORTED_FILE_TYPE`、图片/URL 返回 `PARSER_UNAVAILABLE`（均不重试），容量饱和返回 `PARSE_BUSY`（503，可重试）；`GET /health` 暴露 `parse` 能力块供调用方预先判断类型支持
- **引擎容量有界**：每次引擎调用都是独立进程，故所有 spawn 经全局并发闸门（`GBRAIN_CLI_CONCURRENCY`）+ 有界排队——饱和时返回可重试的 `503 UPSTREAM_BUSY`，而非在批量导入下雪崩
- **知识图谱自动维护（可选）**：开启梦境周期即自动提取页面关系建图（轻量档零 LLM）；管理面 API 手工触发与运行互斥
